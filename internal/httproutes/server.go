...
// SetHandler swaps the handler used to serve incoming requests. Pass nil to
// indicate the plugin is not yet configured; the server will then return 503.
...
// Handle translates the SDK's HandleHTTPRequest into an http.Request, dispatches
// it through the configured handler, and packages the response.
